    ...
    return (
        <div className="relative h-[300px] cursor-pointer group">
            ...
            <span
                ...
                className="absolute z-10 invisible w-8 h-full transition-all opacity-0 cursor-pointer right-5 top-5 group-hover:opacity-100 group-hover:visible"
            >
                <svg
                    width="42"
                    height="38"
            ...
            <button
                onClick={() => addToCart({ url, isLike, id })}
                className="absolute invisible p-4 px-6 py-3 text-sm font-medium text-black transition-all bg-white rounded-lg opacity-0 bottom-5 left-2/4 -translate-x-2/4 group-hover:opacity-100 group-hover:visible"
            >
                Add to cart
            </button>
        </div>
